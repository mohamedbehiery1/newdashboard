...
import { loadCanceledOrders ,pageChanged } from "src/store/orders";
...
const CanceledList = () => {
  ...
  useEffect(() => {
      dispatch(loadCanceledOrders(1, type));    
  }, [type]);
  ...
  const onPageChange = targetPage => {
    ...
    if (actualTargetPage < page) {
      ...
      if (actualTargetPage > numberOfPagesLoaded) {
        // not loaded before
        dispatch(loadCanceledOrders(actualTargetPage, type));
      } else {
        // loaded before
        dispatch(pageChanged({ page: actualTargetPage }));
        return;
    ...
    <>
      ...
      >
        <Container maxWidth={false}>
          <OrderListToolbar pageType='canceled' />
