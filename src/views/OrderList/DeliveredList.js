...
import { loadDeliveredOrders ,pageChanged } from "src/store/orders";
...
const DeliveredList = () => {
  ...
  useEffect(() => {
      dispatch(loadDeliveredOrders(1, type));    
  }, [type]);
  ...
  const onPageChange = targetPage => {
    ...
    if (actualTargetPage < page) {
      ...
      if (actualTargetPage > numberOfPagesLoaded) {
        // not loaded before
        dispatch(loadDeliveredOrders(actualTargetPage, type));
      } else {
        // loaded before
        dispatch(pageChanged({ page: actualTargetPage }));
        return;
    ...
    <>
      ...
      >
        <Container maxWidth={false}>
          <OrderListToolbar pageType='delivered' />
