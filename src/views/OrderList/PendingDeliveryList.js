...
import { loadOrders, loadPendingOrders ,pageChanged } from "src/store/orders";
import { useNavigate , useParams } from "react-router-dom";
import { useTranslation } from 'react-i18next';
import { set, omit, isEmpty, find, forEach, slice } from 'lodash';

const BendingDeliveryList = () => {
  ...
  useEffect(() => {
      dispatch(loadPendingOrders(1, type));    
  }, [type]);
  ...
  const onPageChange = targetPage => {
    ...
    if (actualTargetPage < page) {
      ...
      if (actualTargetPage > numberOfPagesLoaded) {
        // not loaded before
        dispatch(loadPendingOrders(actualTargetPage, type));
      } else {
        // loaded before
        dispatch(pageChanged({ page: actualTargetPage }));
        return;
    ...
    <>
      ...
      >
        <Container maxWidth={false}>
          <OrderListToolbar pageType='pending-delivery' />
...
export default BendingDeliveryList;
