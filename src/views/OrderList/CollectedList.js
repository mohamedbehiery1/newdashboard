...
import { loadCollectedOrders ,pageChanged } from "src/store/orders";
...
const CollectedList = () => {
  ...
  useEffect(() => {
      dispatch(loadCollectedOrders(1, type));    
  }, [type]);
  ...
  const onPageChange = targetPage => {
    ...
    if (actualTargetPage < page) {
      ...
      if (actualTargetPage > numberOfPagesLoaded) {
        // not loaded before
        dispatch(loadCollectedOrders(actualTargetPage, type));
      } else {
        // loaded before
        dispatch(pageChanged({ page: actualTargetPage }));
        return;
    ...
    <>
      ...
      >
        <Container maxWidth={false}>
          <OrderListToolbar pageType='collected' />
