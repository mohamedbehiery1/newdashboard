...
import CarBrandListResults from "./components/CarBrandListResults";
import CarBrandListToolbar from "./components/CarBrandListToolbar";
import { $APP_NAME } from "src/constants";
import { useSelector, useDispatch } from "react-redux";
import { loadCarBrands, carBrandRemoved, pageChanged } from 'src/store/carBrands';
...
import CarBrandService from "src/__services__/CarBrandService";
...
const CarBrandList = () => {
  ...
  // Pagination
  useEffect(() => {
    dispatch(loadCarBrands(1));
  }, []);

  // Pagination
  const { list, loading, rowCount, pageSize, page } = useSelector(state => state.entities.carBrands);
  ...
  const onPageChange = targetPage => {
    ...
    if (actualTargetPage < page) {
      ...
      if (actualTargetPage > numberOfPagesLoaded) {
        // not loaded before
        dispatch(loadCarBrands(actualTargetPage))
      } else {
        // loaded before
        dispatch(pageChanged({ page: actualTargetPage }));
        return;
  ...
  return (
    <>
      <Helmet>
        <title>Car Brand | {$APP_NAME}</title>
      </Helmet>
      <Box
        sx={{
      ...
      >
        <Container maxWidth={false}>
          ...
          <ConfirmationDialog
            ...
            message={t("Car Brand will be deleted")}
